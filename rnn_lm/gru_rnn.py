...
class GruRnn(object):
    ...
    def recurrent_step(self, x_t, h_t_minus_1):
        ...
        r = T.nnet.sigmoid(self.t_Wr[:, x_t] + T.dot(self.t_Ur, h_t_minus_1))
        # calc candidate next hidden state (with reset)
        h_t_candidate = T.tanh(self.t_Wx[:, x_t] + T.dot(self.t_Ux, r * h_t_minus_1))

        # calc update gate activation 
        z = T.nnet.sigmoid(self.t_Wz[:, x_t] + T.dot(self.t_Uz, h_t_minus_1))
